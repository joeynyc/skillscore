...
export class MarkdownReporter {
  generateReport(score: SkillScore): string {
    ...
    // Header
    lines.push('# 📊 SkillScore Evaluation Report');
    lines.push('');
    
    // Skill Info
    lines.push('## 📋 Skill Information');
    lines.push('');
    ...
    lines.push(`**SKILL.md:** ${score.skill.structure.hasSkillMd ? '✅ Present' : '❌ Missing'}`);
    lines.push('');
    ...
    if (moderate.length > 0) {
      lines.push('### 🔄 Good but Could Improve');
      lines.push('');
    ...
    if (failedFindings.length > 0) {
      ...
      lines.push('🎉 No critical issues found! Consider the improvement suggestions above.');
    }
    ...
    lines.push('---');
    lines.push('');
    lines.push(`*Generated by SkillScore v1.0.0 on ${score.timestamp.toLocaleString()}*`);
    lines.push('');
